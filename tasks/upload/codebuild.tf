...
resource "null_resource" "build_and_push" {
  ...
  provisioner "local-exec" {
   command = "./load.sh ${"."} ${"242668925814.dkr.ecr.us-east-1.amazonaws.com/poctest"}:${var.DOCKER_IMAGE_TAG} ${var.AWS_REGION} ${var.AWS_ACCESS_KEY} ${var.AWS_SECRET_KEY}  >> completed.txt"
  }
 }